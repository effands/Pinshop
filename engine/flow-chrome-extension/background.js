...
function _extractUserEmail(jwtToken) {
  try {
    ...
    return parsed.email || parsed.name || parsed.sub || null;
  } catch (e) {
    return null;
  }
}

async function _getProfileLabel() {
  try {
    const tabs = await chrome.tabs.query({ url: 'https://labs.google/*' });
    for (const tab of tabs) {
      if (tab.title && !tab.title.startsWith('http') && tab.title !== 'New Tab') {
        const clean = tab.title.replace(/\s*[-–|].*$/, '').trim();
...
async function _updateInstanceName(userEmail = null) {
  const shortId = instanceId ? instanceId.slice(0, 8) : 'default';
  const email = userEmail || _extractUserEmail(flowKey);
  await _detectProjectIdFromTabs();
  if (email && typeof email === 'string' && email.includes('@')) {
    instanceName = `Chrome (${email})`;
  } else {
    const profileLabel = await _getProfileLabel();
    instanceName = profileLabel ? `Chrome (${profileLabel} · ${shortId})` : `Chrome (${shortId})`;
  }
  chrome.storage.local.set({ instanceName, currentProjectId });
...
async function init() {
  const data = await chrome.storage.local.get(['flowKey', 'metrics', 'callbackSecret', 'callbackUrl', 'instanceId', 'instanceName', 'currentProjectId']);
  instanceId = data.instanceId || crypto.randomUUID();
  if (data.flowKey) flowKey = data.flowKey;
  if (data.currentProjectId) currentProjectId = data.currentProjectId;
  
  await _detectProjectIdFromTabs();
  const email = _extractUserEmail(flowKey);
  const shortId = instanceId.slice(0, 8);
  if (email && email.includes('@')) {
    instanceName = `Chrome (${email})`;
  } else {
    const profileLabel = await _getProfileLabel();
    instanceName = profileLabel ? `Chrome (${profileLabel} · ${shortId})` : (data.instanceName || `Chrome ${shortId}`);
  }

  await chrome.storage.local.set({ instanceId, instanceName, currentProjectId });
...
chrome.webRequest.onBeforeSendHeaders.addListener(
  (details) => {
    ...
    console.log('[Flow Agent] Bearer token captured for instance:', instanceName);
...
async function captureTokenFromFlowTab(forceReload = false) {
  ...
  if (!tabs.length) {
    const loginTabs = await chrome.tabs.query({ url: 'https://accounts.google.com/*' });
    const pendingLogin = loginTabs.find((tab) => isFlowLoginUrl(tab.url));
    if (pendingLogin) {
      await chrome.tabs.update(pendingLogin.id, { active: true });
      console.warn('[Flow Agent] Google Flow login is still waiting for the user');
      return;
    }
    if (_openingFlowTab) {
  ...
  try {
    if (forceReload) {
      ...
      console.log('[Flow Agent] Reloaded Flow tab to request fresh OAuth token');
      return;
    }
    await chrome.scripting.executeScript({
...
async function refreshExpiredFlowToken(staleToken) {
  if (flowKey === staleToken) flowKey = null;
  metrics.tokenCapturedAt = null;
  await chrome.storage.local.remove('flowKey');
  await captureTokenFromFlowTab(true);
  for (let i = 0; i < 20; i++) {
    if (flowKey && flowKey !== staleToken) return flowKey;
    await sleep(500);
  }
  return null;
}
    ...
    try {
      ...
      if (msg.method === 'api_request') {
        ...
        if (tabs.length) {
          ...
          // No tab — open one in background without stealing window focus
          await chrome.tabs.create({ url: 'https://labs.google/fx/tools/flow', active: false });
...
async function requestCaptchaFromTab(tabId, requestId, pageAction) {
  try {
    ...
    if (tab?.url && (tab.url.includes('accounts.google.com') || !tab.url.includes('labs.google'))) {
      return { error: 'FLOW_LOGIN_REQUIRED: Tab Google Flow belum terhubung/login di Chrome.' };
    }
    ...
    if (msg.includes('Cannot access contents') || msg.includes('permission')) {
      return { error: 'FLOW_LOGIN_REQUIRED: Google Flow membutuhkan Login Google. Buka tab Google Flow di Chrome.' };
    }
    console.warn(`[Flow Agent] sendMessage initial attempt failed on tab ${tabId}: ${msg}`);
    try {
      const tab = await chrome.tabs.get(tabId).catch(() => null);
      if (tab?.url && (tab.url.includes('accounts.google.com') || !tab.url.includes('labs.google'))) {
        return { error: 'FLOW_LOGIN_REQUIRED: Tab Google Flow belum terhubung/login di Chrome.' };
      ...
      if (msg2.includes('Cannot access contents') || msg2.includes('permission')) {
        return { error: 'FLOW_LOGIN_REQUIRED: Tab Google Flow belum login / memerlukan otorisasi di Chrome.' };
      }
      console.error(`[Flow Agent] executeScript re-injection failed on tab ${tabId}: ${msg2}`);
      return { error: `CAPTCHA_FAILED: ${msg2}` };
    }
  }
}
...
async function solveCaptcha(requestId, captchaAction) {
  ...
  if (!tabs.length) {
    ...
    if (pendingLogin) {
      await chrome.tabs.update(pendingLogin.id, { active: true });
      return { error: 'FLOW_LOGIN_REQUIRED: selesaikan login Google pada tab yang dibuka' };
    }
    // Auto-open Flow visibly so login/consent cannot remain hidden.
    try {
...
async function handleApiRequest(msg) {
  ...
  try {
    ...
    if (shouldRetryFlowAuth(response.status, 0)) {
      console.warn('[Flow Agent] OAuth token rejected; refreshing from Flow tab and retrying once');
      const refreshedToken = await refreshExpiredFlowToken(activeFlowKey);
      if (refreshedToken) {
        fetchHeaders['authorization'] = `Bearer ${refreshedToken}`;
    ...
    const remainingCredits = FlowCredit.extractRemainingCredits(responseData);
    if (remainingCredits !== null) {
      metrics.remainingCredits = remainingCredits;
      chrome.storage.local.set({ metrics });
      broadcastStatus();
    }
...
async function refreshRemainingCredits() {
  ...
  const response = await fetch(`https://aisandbox-pa.googleapis.com/v1/credits?key=${API_KEY}`, {
    method: 'GET',
    headers: { authorization: `Bearer ${flowKey}` },
    credentials: 'include',
  });
  if (!response.ok) return { error: `API_${response.status}` };
  const data = await response.json();
  const remainingCredits = FlowCredit.extractRemainingCredits(data);
  if (remainingCredits === null) return { error: 'CREDITS_MISSING' };
  metrics.remainingCredits = remainingCredits;
  await chrome.storage.local.set({ metrics });
  broadcastStatus();
  return { ok: true, remainingCredits };
}
...
chrome.runtime.onMessage.addListener((msg, _, reply) => {
  ...
  if (msg.type === 'REFRESH_CREDITS') {
    refreshRemainingCredits()
      .then(reply)
      .catch((e) => reply({ error: e.message || 'CREDITS_FETCH_FAILED' }));
    return true;
  }
